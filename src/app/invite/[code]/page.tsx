import { getCircleByInviteCode, joinCircleByCode, getCircle } from "@/app/actions/circles";
...
export default async function InvitePage({ params }: InvitePageProps) {
    ...
    return (
      <main className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-6 text-center relative overflow-hidden">
        ...
        <div className="relative z-10 animate-fade-in-up">
            <h1 className="text-4xl font-bold mb-4 tracking-tight">Invalid Invite</h1>
            <p className="text-muted-foreground mb-8 text-lg text-balance">
            This invite link looks invalid or has expired.
            </p>
            <Link href="/">
            <Button className="rounded-full px-8 h-12" size="lg">Return Home</Button>
            </Link>
        </div>
      </main>
    );
  ...
  const ownerName = circlePreview.owner.name || "Circle Owner";
  ...
  return (
    <main className="min-h-screen bg-black text-white flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="max-w-md w-full glass rounded-[2.5rem] border-white/10 p-8 md:p-12 text-center relative z-10 shadow-2xl animate-fade-in-up">
        {/* Circle Avatar */}
        <div className="w-24 h-24 sm:w-32 sm:h-32 mx-auto mb-8 rounded-[2rem] bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center text-5xl font-bold text-white shadow-2xl shadow-purple-900/40 ring-1 ring-white/10">
        ...
        <div className="flex items-center justify-center gap-2 mb-8 text-muted-foreground text-lg">
           <span className="w-6 h-6 rounded-full bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center text-[10px] font-bold text-white shadow-lg">
                {ownerInitial}
           </span>
            <p>
                Invited by <span className="font-semibold text-white">{ownerName}</span>
            </p>
        </div>
        ...
        {existingMembership ? (
           <div className="space-y-4">
              {existingMembership.currentUserStatus === "PENDING" ? (
                ...
                <div className="bg-green-500/10 border border-green-500/20 rounded-2xl p-4 mb-4">
                   ...
                   <p className="text-sm text-green-500/80">
                       You are already part of this circle.
                   </p>
                </div>
              )}
              ...
              <Link href={`/circle/${circlePreview.id}`}>
                 <Button
                    size="lg"
                    className="w-full rounded-full h-14 text-lg font-bold shadow-xl shadow-purple-500/20 bg-white text-black hover:bg-zinc-200 hover:scale-[1.02] active:scale-[0.98] transition-all duration-300"
                 >
                    {existingMembership.currentUserStatus === "PENDING" ? "View Status" : "Go to Circle"}
          ...
          <form action={joinAction} className="space-y-4">
            ...
            >
             ...
             <p className="text-xs text-zinc-500">
              You will join as <span className="text-zinc-300">{session.user.name}</span>
            </p>
          </form>
        )}
        ...
        {/* Footer Link */}
         <div className="mt-8 pt-6 border-t border-white/5">
            <Link href="/" className="text-sm text-zinc-500 hover:text-white transition-colors">
                {existingMembership ? "Back to Dashboard" : "No thanks, return to Dashboard"}
            </Link>
         </div>
      </div>
    </main>
